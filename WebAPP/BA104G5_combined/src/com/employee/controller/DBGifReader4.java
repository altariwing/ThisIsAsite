...
public class DBGifReader4 extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			System.out.println("SELECT EMP_PHOTO FROM EMPLOYEE WHERE EMPNO = '"+str2+"'");
			ResultSet rs = stmt.executeQuery(
					"SELECT EMP_PHOTO FROM EMPLOYEE WHERE EMPNO = '"+str2+"'");
			...
			//null ec
			InputStream in = getServletContext().getResourceAsStream("/NoData/nopic2.jpg");
			byte[] buf = new byte[in.available()];
			in.read(buf);
			out.write(buf);
			in.close();
