...
//			+ "VALUES ('MB'||(LPAD(to_char(MEM_SEQ.NEXTVAL),8,'0')),?,?,?,?,?,?,?,?)";
	private static final String INSERT_STMT = 
			"INSERT INTO Member (mem_no,mem_id,mem_psw,mem_name,mem_addr,mem_game,search_state,lock_state) "
			+ "VALUES ('MB'||(LPAD(to_char(MEM_SEQ.NEXTVAL),8,'0')),?,?,?,?,?,?,?)";
	//一般的 GET 時不抓密碼
...
//			pstmt.setBytes(5, memVO.getMem_img());
			...
			pstmt.setString(7, "OFF"); // 此為 lock_state，新加入為 OFF，通過 e-mail 驗證才會改為 On
